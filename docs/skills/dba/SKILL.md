...
5. **Contexto de Base de Datos y Sesión:**
   - **IMPORTANTE:** Como la cadena de conexión administrativa apunta a `master`, todos tus scripts SQL deben empezar explícitamente con `USE [NombreDeLaBaseDeDatos];` (ej. `USE [{DatabaseName}];`) seguido de `GO`.
...
8. **Ejecución Automática contra SQL Server (100% Autónoma):**
   ...
   - **Paso B:** Busca el archivo `appsettings.json` de persistencia en `Src/{ProjectName}/{ProjectName}.Infrastructure/Persistence/appsettings.json`.
   - **Paso C:** Usa el comando `run_command` para ejecutar `run_sql.ps1` apuntando a ese archivo:
     ```powershell
     powershell -File ".agents\skills\dba\scripts\run_sql.ps1" -ScriptFile "docs\sql\<NombreTabla>.sql" -AppSettingsPath "Src\{ProjectName}\{ProjectName}.Infrastructure\Persistence\appsettings.json" -ConnectionName "master"
     ```
...
**Input:** "Ejecuta el script para la tabla Users en la base de datos {ProjectName}."
...
3. Ejecuta la herramienta `run_command` con la configuración de Infraestructura:
   `powershell -File ".agents\skills\dba\scripts\run_sql.ps1" -ScriptFile "docs\sql\Usuarios.sql" -AppSettingsPath "Src\{ProjectName}\{ProjectName}.Infrastructure\Persistencia\appsettings.json" -ConnectionName "master"`
**Output al usuario:**
"✅ He utilizado las credenciales administrativas de Infraestructura para ejecutar el script exitosamente en la base de datos {ProjectName}."
...
- ✅ **Scripts Seguros:** Asegura que tus scripts incluyan `USE [{DatabaseName}];` para no crear tablas por error en la base de datos `master`.
